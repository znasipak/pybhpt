...
void save_metric_tetrad_data(std::string filename, Vector t, Vector r, Vector z, Vector phi, ComplexVector hab){
	...
	file << "t\tr\tz\tphi\tRe[hab]\tIm[hab]\n";

	for(size_t i = 0; i < hab.size(); i++) {
    sprintf(buff, "%.15e\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\n", t[i], r[i], z[i], phi[i], std::real(hab[i]), std::imag(hab[i]));
    file << buff;
	}
	file.close();
}
...
void save_metric_data(std::string filename, Vector t, Vector r, Vector z, Vector phi, Vector hab){
	...
	for(size_t i = 0; i < hab.size(); i++) {
    sprintf(buff, "%.15e\t%.15e\t%.15e\t%.15e\t%.15e\n", t[i], r[i], z[i], phi[i], hab[i]);
    file << buff;
	}
	file.close();
}

void save_full_metric_data(std::string filename, Vector t, Vector r, Vector z, Vector phi, RealMatrix hmunu){
	char buff[500];
	...
	file << "t\tr\tz\tphi\thtt\thtr\thtz\thtphi\thrr\thrz\thrphi\thzz\thzphi\thphiphi\n";

	for(size_t i = 0; i < hmunu[0].size(); i++) {
    sprintf(buff, "%.15e\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\t%.15e\n", t[i], r[i], z[i], phi[i], hmunu[0][i], hmunu[1][i], hmunu[2][i], hmunu[3][i], hmunu[4][i], hmunu[5][i], hmunu[6][i], hmunu[7][i], hmunu[8][i], hmunu[9][i]);
    file << buff;
	}
	file.close();
}

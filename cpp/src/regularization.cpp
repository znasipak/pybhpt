...
int save_regAB_data(GeodesicSource geo, const int &sampleSize, const std::string &dir){
  Vector At = regularization_parameter_At_from_source(geo, sampleSize);
  Vector Ar = regularization_parameter_Ar_from_source(geo, sampleSize);
  Vector Ath(At.size());
  Vector Aph(At.size());

  Vector Bt = regularization_parameter_Bt_from_source(geo, sampleSize);
  Vector Br = regularization_parameter_Br_from_source(geo, sampleSize);
  Vector Bth = regularization_parameter_Btheta_from_source(geo, sampleSize);
  Vector Bph = regularization_parameter_Bphi_from_source(geo, sampleSize);

  save_regA_data(At, Ar, Ath, Aph, geo, dir);
  save_regB_data(Bt, Br, Bth, Bph, geo, dir);
...
int save_reg_data(Vector regT, Vector regR, Vector regTh, Vector regPh, GeodesicSource geo, const std::string &filename, const std::string &dir){
  ...
  return save_reg_data_generic(regT, regR, regTh, regPh, geo, filename, dir);
}
...
int save_reg_data_circular(Vector regT, Vector regR, Vector regTh, Vector regPh, GeodesicSource geo, const std::string &filename, const std::string &dir){
  ...
  if(dir.back() == '/'){
	...
	std::ofstream file;
	file.open(filepath);
	file << "r\ttheta\tregT\tregR\tregTheta\tregPhi\n";

  file << geo.getRadialPosition()[0] << "\t" << geo.getPolarPosition()[0] << "\t" << regT[0] << "\t" << regR[0] << "\t" << regTh[0] << "\t" << regPh[0] << "\n";
...
int save_reg_data_equatorial(Vector regT, Vector regR, Vector regTh, Vector regPh, GeodesicSource geo, const std::string &filename, const std::string &dir){
  ...
  int sampleReg = regT.size();
	std::ofstream file;
	file.open(filepath);
	file << "r\ttheta\tregT\tregR\tregTheta\tregPhi\n";

  for(int jr = 0; jr <= sampleReg/2; jr++){
    file << std::scientific << std::setprecision(15);
  	file << geo.getRadialPosition()[2*jr*Nr/sampleReg] << "\t" << geo.getPolarPosition()[0] << "\t" << regT[jr] << "\t" << regR[jr] << "\t" << regTh[jr] << "\t" << regPh[jr] << "\n";
  }

  for(int jr = sampleReg/2 + 1; jr < sampleReg; jr++){
    file << std::scientific << std::setprecision(15);
  	file << geo.getRadialPosition()[Nr - 2*(jr - sampleReg/2)*Nr/sampleReg] << "\t" << geo.getPolarPosition()[0] << "\t" << regT[jr] << "\t" << regR[jr] << "\t" << regTh[jr] << "\t" << regPh[jr] << "\n";
  }
...
int save_reg_data_spherical(Vector regT, Vector regR, Vector regTh, Vector regPh, GeodesicSource geo, const std::string &filename, const std::string &dir){
  ...
  for(int jth = 0; jth <= sampleReg/2; jth++){
    file << std::scientific << std::setprecision(15);
    file << geo.getRadialPosition()[0] << "\t" << geo.getPolarPosition()[2*jth*Nth/sampleReg] << "\t" << regT[jth] << "\t" << regR[jth] << "\t" << regTh[jth] << "\t" << regPh[jth] << "\n";
  }

  for(int jth = sampleReg/2 + 1; jth < sampleReg; jth++){
    file << std::scientific << std::setprecision(15);
    file << geo.getRadialPosition()[0] << "\t" << geo.getPolarPosition()[Nth - 2*(jth - sampleReg/2)*Nth/sampleReg]  << "\t" << regT[jth] << "\t" << regR[jth] << "\t" << regTh[jth] << "\t" << regPh[jth] << "\n";
  }
...
int save_reg_data_generic(Vector regT, Vector regR, Vector regTh, Vector regPh, GeodesicSource geo, const std::string &filename, const std::string &dir){
  ...
  for(int jr = 0; jr <= sampleReg/2; jr++){
    for(int jth = 0; jth <= sampleReg/2; jth++){
      file << std::scientific << std::setprecision(15);
      file << geo.getRadialPosition()[2*jr*Nr/sampleReg] << "\t" << geo.getPolarPosition()[2*jth*Nth/sampleReg] << "\t" << regT[jr*sampleReg + jth] << "\t" << regR[jr*sampleReg + jth] << "\t" << regTh[jr*sampleReg + jth] << "\t" << regPh[jr*sampleReg + jth] << "\n";
    }

    for(int jth = sampleReg/2 + 1; jth < sampleReg; jth++){
      file << std::scientific << std::setprecision(15);
      file << geo.getRadialPosition()[2*jr*Nr/sampleReg] << "\t" << geo.getPolarPosition()[Nth - 2*(jth - sampleReg/2)*Nth/sampleReg]  << "\t" << regT[jr*sampleReg + jth] << "\t" << regR[jr*sampleReg + jth] << "\t" << regTh[jr*sampleReg + jth] << "\t" << regPh[jr*sampleReg + jth] << "\n";
    }
    file << "\n";
  }
  ...
  for(int jr = sampleReg/2 + 1; jr < sampleReg; jr++){
    for(int jth = 0; jth <= sampleReg/2; jth++){
      file << std::scientific << std::setprecision(15);
      file << geo.getRadialPosition()[Nr - 2*(jr - sampleReg/2)*Nr/sampleReg] << "\t" << geo.getPolarPosition()[2*jth*Nth/sampleReg] << "\t" << regT[jr*sampleReg + jth] << "\t" << regR[jr*sampleReg + jth] << "\t" << regTh[jr*sampleReg + jth] << "\t" << regPh[jr*sampleReg + jth] << "\n";
    }

    for(int jth = sampleReg/2 + 1; jth < sampleReg; jth++){
      file << std::scientific << std::setprecision(15);
      file << geo.getRadialPosition()[Nr - 2*(jr - sampleReg/2)*Nr/sampleReg] << "\t" << geo.getPolarPosition()[Nth - 2*(jth - sampleReg/2)*Nth/sampleReg]  << "\t" << regT[jr*sampleReg + jth] << "\t" << regR[jr*sampleReg + jth] << "\t" << regTh[jr*sampleReg + jth] << "\t" << regPh[jr*sampleReg + jth] << "\n";
    }
    file << "\n";
  }
